...
Local port of the workshop's `900-sandboxed-code-exec/code-executor-mcp/server.py`. The
only substantive change is DynamoDB -> SQLite in `_fetch_rows` (and the matching change in
query.py); the tool signature, the docstring the model actually reads, the return shape, and
the /chart/{id} custom route are unchanged.

Exposes ONE tool, run_python, that:
  1. builds a scoped, parameterized SQL query from the LLM's params,
  2. fetches the bounded orders slice (this pod holds the only DB handle),
  3. vends a fresh air-gapped gVisor sandbox and injects the rows + code as
     files (out-of-band — the data never passes through the LLM),
  4. runs the untrusted code in the sandbox and returns capped stdout.
...
The sandbox has no network and no credentials; all trust lives in this broker.
"""
...
def _fetch_rows(**params) -> list[dict]:
    """Run the scoped query and return plain order dicts."""
    sql, args = build_query(**params)
    rows: list[dict] = []
    for r in _CONN.execute(sql, args):
        row = dict(r)
        # `items` is stored as a JSON string, exactly as DynamoDB held it. Parse it back to
        # a list so the sandbox sees the shape the run_python docstring promises
        # (list of {name, qty, price}). Mirrors lookup_order in the 500 mcp-server.
        if isinstance(row.get("items"), str):
            row["items"] = json.loads(row["items"])
        # `total` is likewise a String column; hand the sandbox a float so pandas sums it
        # instead of concatenating strings.
        if isinstance(row.get("total"), str) and row["total"]:
            row["total"] = float(row["total"])
        # Absent tracking is stored as "" by the seeder; DynamoDB simply had no attribute.
        # Drop the key so the docstring's "ABSENT for processing/cancelled" stays true.
        if row.get("tracking") in ("", "None", None):
            row.pop("tracking", None)
        rows.append(row)
    return rows
...
def run_python(
    ...
    """Run Python against a scoped slice of the orders dataset in an isolated sandbox.
    ...
    rows. The code runs kernel-isolated (gVisor sandbox), air-gapped, with
