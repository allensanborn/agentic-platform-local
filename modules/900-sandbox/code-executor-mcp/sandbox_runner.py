"""Vend a gVisor sandbox, run untrusted code against injected data, terminate.
...
injected as a file (out-of-band), never through the LLM. The sandbox is
air-gapped and holds no credentials; this process holds the SandboxClaim RBAC.

Copied from the workshop verbatim except for the WARMPOOL default name and the words
"sandbox" -> "sandbox" (ADR 0005). The single-use lifecycle is the point: claim, run,
destroy, pool replaces.
"""
...
WARMPOOL = os.environ.get("SANDBOX_WARMPOOL", "gvisor-python-pool")
...
def run_in_sandbox(code: str, rows: list[dict]) -> dict:
    """Run `code` in a fresh air-gapped sandbox with `rows` at /app/orders.json.
